...
pA = data{11}.extractTimetable;
pB = data{13}.extractTimetable;

p1_f = pA_filter.Variables * 10^2;
p2_f = pB_filter.Variables * 10^2;
p1 = pA.Variables * 10^2;
p2 = pB.Variables * 10^2;
...
plot(piston_pos.Variables, Fhyd,'k-','LineWidth',1.5,'DisplayName','$\Omega(t)$');
xlabel('Piston travel [m]')
ylabel('$F_{hyd}$ $[kN]$')
...
F_hyd = p_A * A_piston - p_B * A_rod;  % Measured hydraulic force [N]
...
m_beam  = 415.74;                       % Beam mass [kg]
g       = 9.81;                         % Gravitational acceleration [m/s^2]
x_cg    = 3.01315;                      % Distance to center of gravity [m]
...
% Sweep beam angle
theta = deg2rad(linspace(-16.6, 30, 200));
...
for i = 1:length(theta)
    ...
    F_cyl_theo(i) = m_beam * g * x_cg / moment_arm;
